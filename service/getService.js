...
const PATH = 'D:/ELAAS/elaas-frontend/src/views/app/expense';
const ROOT_PATH = 'D:/ELAAS/elaas-frontend/src'
...
const readComponentFile = (fileName, file, folder) => {
    ...
    if (fileName === 'IndexView.vue') {
        ...
        if (matchIndex && matchIndex.length > 0) {
            matchIndex = matchIndex.filter(f => f.indexOf('DateService') === -1)

            matchIndex.forEach((m) => {
                const service = m.substring(m.indexOf('{') + 1, m.indexOf('}'))
                const path = m.substring(m.indexOf('\'')).replace(/\'/g, '').replace('@/', `${ROOT_PATH}/`)
        ...
        if (matchFiles) {
            matchFiles.forEach((mf, index) => {
                ...
                const path = `${ROOT_PATH}/views/${mPath}/${mFile}.vue`;
                    ...
                    if (matchSubComponent) {
                        matchSubComponent.forEach((mComponent) => {
                            ...
                            if (_mComponent.indexOf('./') !== -1) {
                                _mComponent = `${ROOT_PATH}/views/${mPath}/${_mComponent.replace(/.\//, '')}`
                            } else {
                                _mComponent = _mComponent.replace('@/', `${ROOT_PATH}/`)
                            }
                            ...
                            if (matchSubContent && matchSubContent.length > 0) {
                                ...
                                matchSubContent = matchSubContent.filter(f => f.indexOf('DateService') === -1)
